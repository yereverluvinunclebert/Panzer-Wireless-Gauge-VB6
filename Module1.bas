...
'Public gblPercentWireless As String
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Wireless-Gauge-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblFormHighDpiXPosTwips = fGetINISetting("Software\PzWirelessGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblFormHighDpiYPosTwips = fGetINISetting("Software\PzWirelessGauge", "formHighDpiYPosTwips", gblSettingsFile)
...
'        End If
    Else
        gblFormLowDpiXPosTwips = fGetINISetting("Software\PzWirelessGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblFormLowDpiYPosTwips = fGetINISetting("Software\PzWirelessGauge", "formLowDpiYPosTwips", gblSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzWirelessGauge", "formHighDpiXPosTwips", gblFormHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzWirelessGauge", "formHighDpiYPosTwips", gblFormHighDpiYPosTwips, gblSettingsFile
        Else
            gblFormLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblFormLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzWirelessGauge", "formLowDpiXPosTwips", gblFormLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzWirelessGauge", "formLowDpiYPosTwips", gblFormLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hwnd, "open", thisCommand, "Panzer Wireless Gauge.exe prefs", "", 1)
    Else
